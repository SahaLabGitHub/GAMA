...
foreach my $file (@ARGV){
  ...
  my $box_size = 2; #used at line no approx 208
  my $int_radius = 10; #used at line no approx 652
...
#  Changing the index by plus one of groups elements
   foreach my $c(0..$#gr){
     foreach my $d(0..$#{$gr[$c]}){
       my $tm = $gr[$c][$d] + 1;
       $gr[$c][$d] = $tm;
  ...
  START:
  foreach my $c(0..$#gr){
    if ($#carry>0 && $c+1 ~~ @carry){
       next;
    }
    #print "Etttttttttttntering line @{$gr[$c]}\n";
    foreach my $d(0..$#{$gr[$c]}){
      #print "#################\n";
      foreach my $e(0..$#gr){
        if ($gr[$c][$d] ~~ @{$gr[$e]}){
    ...
    my @unique = unique(@ubag);
    splice @ubag;
    if ($#unique == $#{$gr[$c]}){
      #print "This is Done and it is @{$gr[$c]} ($#{$gr[$c]})\n";
      print MYHANDLE "This is Done and it is @{$gr[$c]} ($#{$gr[$c]})\n" if ($detailed_output==0);
      push (@groups, @{$gr[$c]});
      push (@{$groupsAoA[$c]}, @{$gr[$c]});
    }
    push (@carry, @{$gr[$c]}) if $#unique == $#{$gr[$c]};
    unless ($#unique == $#{$gr[$c]}){
      @{$gr[$c]} = @unique;
      goto START;
    }
  ...
  #say Dumper \@sys;
# New code ended here |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  # For Sanity check of number of atoms and duplicacy in sys array
  my @syscheck = ();
  #my %seen4;
  foreach my $k(0 .. $#sys){
    foreach my $l(0 .. $#{$sys[$k]}){
